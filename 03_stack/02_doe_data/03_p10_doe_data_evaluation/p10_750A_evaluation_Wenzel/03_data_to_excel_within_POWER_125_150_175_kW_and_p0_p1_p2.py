...
h2_flow_n_dot = []
efficiency = []
in_range_dict = {cells: {line: {i: pd.DataFrame() for i in range(3)} for line in [125, 150, 175]} for cells in [350, 400, 450, 500]}
...
for cells in all_cells:
    ...
    for i in range(3):
        ...
        plt.plot(x_sorted, cubic_fit, '-', label=f'p{i}: cubic fit: 1.42 bara +/- 0.18 bara', c=colors[i])
    ...
    #%% lets extract the data for the 3 desired power levels for each pressure group data_subdivided
    vertical_lines = [125, 150, 175]
    buffer = 2 # look +- 2 kW around the vline
    ...
    for line in vertical_lines:
        for i in range(3):
            ...
            corresponding_data["Cells"] = cells
            
            # Store the data in the dictionary
            in_range_dict[cells][line][i] = corresponding_data

# Convert the dictionary to a format suitable for export to Excel
export_dict = {}
for cells, power_data in in_range_dict.items():
    for line, pressure_data in power_data.items():
        for pressure, df in pressure_data.items():
            sheet_name = f"{cells}Cells_{line}p_{pressure}"
            export_dict[sheet_name] = df

# Export to Excel
with pd.ExcelWriter("in_range_data.xlsx") as writer:
