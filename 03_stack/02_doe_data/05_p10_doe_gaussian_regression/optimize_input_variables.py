...
def optimize_input_variables(power_constraint_kW=75.0, specified_cell_count=275, flight_level_100ft=50, consider_turbine=True, end_of_life=False):
    ...
    print(f"  Recirculation Pump Power Estimate at Fligh-Level {flight_level_100ft}: {reci_pump_power_kW:.2f} kW")
    ...
    with open(f'optimized_input_for_{int(power_constraint_kW)}kW_with_{int(specified_cell_count)}_cells.txt', 'w') as file:
        ...
        file.write(f"  Recirculation Pump Power required at Fligh-Level {flight_level_100ft}: {reci_pump_power_kW:.2f} kW")
