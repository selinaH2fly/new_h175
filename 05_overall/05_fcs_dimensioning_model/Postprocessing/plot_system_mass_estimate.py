...
def plot_system_mass_estimate(plot_params, params_general, componentsP_dict,  show_plot, saving=True, mode="bol"):  
    """
    ...
    cell_no = params_general['cellcounts']
    ...
    # Get mass constants from parameters.py
        ...
        for df, title, color, m_stack_value, marker in zip(data, titles, colors, m_stack_values, markers):
            ...
            df = df[(df["eol (t/f)"] == filter_mode) & 
                    (df["Flight Level (100x ft)"] == fl) &
                    (df["weighting ([0,1])"] == weighting)]
            
            if df.empty:
                print(f"No data available for Predicted FCM Mass {title} at FL {fl} or weighting {weighting}. Skipping plot.")
            ...
            for point in points:
                ...
                for key, mean_value in componentsP_dict.items():
        ...
        plot_params.update({'title':f'Predicted FCM Mass, {mode_name}, FL {fl}, Weighting {weighting}'})
        ...
        if saving and ax.collections:
            file_path = create_plot_save_directory(f'Weight_estimation_vs_power_{mode}_fl_{fl}_weighting_{weighting}.png', weighting, fl)
