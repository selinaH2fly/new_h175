...
def plot_power_needs(plot_params, params_general, show_plot, saving=True ):
    """
    ...
    for weighting, fl in itertools.product(weightings, fl_set): 
        for df1, title in zip(data, titles):
            ...
            if df.empty:
                print(f"No data available for Component Powers {title} at FL {fl} or for weighting {weighting}. Skipping plot.")
