...
def plot_power_needs_heatflux(plot_params, params_general, show_plot, saving=True):
    """
    ...
    for weighting, fl in itertools.product(weightings, fl_set): 
        for df1, title in zip(data, titles):
            ...
            df = format_data_for_plot(df1, plot_params['components'], fl, plot_params['power_range'], weighting, eol_col='eol (t/f)')
            
            if df.empty:
                print(f"No data available for Heat Flux {title} at FL {fl} and weighting {weighting}. Skipping plot.")
            ...
            title = f"Heat Flux of Components [kW], {title}, FL {fl}, Weighting {weighting}"
            ax.set_title(title, pad=30, loc='center')
        
            # Save and show the plot
            if saving and ax.texts:
                file_path = create_plot_save_directory(f'Component_Heat_Flux_{title}_fl_{fl}_weighting_{weighting}.png', weighting, fl)
