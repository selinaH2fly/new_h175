...
for current = currents
    ...
    if ~isempty(index)
        dp_mean(current/50, :) = [mean(cathode_pressure_drop_mbar(index)) var(cathode_pressure_drop_mbar(index))];
...
if make_plot
    ...
    % saveas(fig_dp_vs_I_stoich, fullfile('results','cathode_pressure_drop_vs_current_joby.fig'))
    % saveas(fig_dp_vs_I_stoich, fullfile('results','cathode_pressure_drop_vs_current_joby.png'))
end
