...
def initialize(input_dict, result_dict, bc_dict):
    ...
    hvdcdc = ThermSim.Heatsource(7, 8, "hvdcdc")
    ...
    if input_dict is not None:
        for name in input_dict:
            ...
            if name == "throttle_delta_p":
                input_dict[name][0] = tcv1.delta_p_2        # depends on architecture
            if name == "p_in":
    ...
    if "Vdot_1" in result_dict.keys():
        ...
        result_dict["Vdot_1"][2] = "Flow over lcdcdc in [l/s]"
        result_dict["vdot_intercooler"] = result_dict["Vdot_1"]
        del result_dict["Vdot_1"]
    if "Vdot_2" in result_dict.keys():
        result_dict["Vdot_2"][0] = hvdcdc.Vdot_in        # depends on architecture!
        result_dict["Vdot_2"][2] = "Flow over hvdcdc in [l/s]"
