...
def initialize(input_dict, result_dict, bc_dict):
    ...
    """
    #circ.add_bc("%s = 0.15"%intercooler.Vdot_in)
    ...
    if "Vdot_3" in result_dict.keys():      # depends on architecture
        result_dict["Vdot_3"][0] = compressor.Vdot_in        # depends on architecture!
        result_dict["Vdot_3"][2] = "Flow over Compressor in [l/s]"
        result_dict["vdot_compressor"] = result_dict["Vdot_3"]
